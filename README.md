...
Desktop Cleaner 是一个 Windows 桌面入口面板工具。它把桌面上的文件、文件夹、快捷方式和外部路径引用显示在半透明分组面板里，方便分类、查找和启动。
...
desktop_tidy/
  ...
  persistence/                配置读写与迁移
  services/                   桌面索引、图标、右键菜单、启动项和 Windows 集成
  ui/                         面板、项目网格、设置窗口和托盘控制
main.py                       应用入口
...
- 第一次启用桌面接管时会弹出确认提示。
- 程序隐藏后可从系统托盘重新显示面板、打开设置、恢复桌面图标或退出。
- 如果重复启动 exe，新的进程会直接退出，只保留一个正在运行的实例。
